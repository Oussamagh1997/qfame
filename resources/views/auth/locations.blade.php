...
$sel="SELECT location FROM locations";
...
<div class="container">
    <div class="navbar">
        ...
        <ul class="nav nav-pills nav-fill">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="/ousama/public/auth/locations">Locations</a>
            </li>
            <li class="nav-item">
    ...
    <div class="row" style="height:65vh;">
        <div class="col-md-4">
            <form method="POST" action="{{ url('/auth/locations')}}">
                @csrf
                <select required size="20" class='scroll' id="loc" name="loc" >
                    ...
                    <select required size="20" class='scroll' id="event" name="event">
                        @if(isset($location))
                            @foreach($location as $l)
                                <option value="{{$l->eventid}}//-{{$l-> type_desc}}//-{{ $l->location }}//-{{ $l->year }}" >{{ $l->year }} - {{ $l->type_desc }}</option>
                            @endforeach
                        @endif
                        <br><br>
                        <input class="btn-primary btn w-100 mt-4" type="submit">
